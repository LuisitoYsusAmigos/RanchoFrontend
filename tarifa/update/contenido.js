...
window.onload = async function () {
    try {
        ...
        const response = await fetch(`https://ranchoback.api.dev.dtt.tja.ucb.edu.bo/cliente/edit/${catastroId}`, {
            method: 'PUT',
            headers: {
        ...
        if(data.status==true){
            //alert(data.ci)
            document.getElementById('clienteForm').action = `https://ranchoback.api.dev.dtt.tja.ucb.edu.bo/cliente/update/${catastroId}`;
            data = data.value[0]
            console.log(data);
