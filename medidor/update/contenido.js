...
window.onload = async function () {
    try {
        ...
        const response = await fetch(`http://localhost:3000/medidor/edit/${catastroId}`, {
            method: 'PUT',
            headers: {
        ...
        if (data.status == true) {
            //alert(data.ci)
            document.getElementById('medidorForm').action = `http://localhost:3000/medidor/update/${catastroId}`;
            data = data.value[0]
           // console.log(data);
