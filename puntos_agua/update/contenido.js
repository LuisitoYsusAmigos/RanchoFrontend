...
window.onload = async function () {
    try {
        ...
        const response = await fetch(`https://ranchoback.api.dev.dtt.tja.ucb.edu.bo/puntosAgua/edit/${catastroId}`, {
            method: 'PUT',
            headers: {
        ...
        if (data.status === true) {
            document.getElementById('puntosAguaForm').action = `https://ranchoback.api.dev.dtt.tja.ucb.edu.bo/puntosAgua/update/${catastroId}`;
            data = data.value[0];
            console.log(data);
